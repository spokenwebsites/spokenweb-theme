		...
		<div class="col-md-10 col-sm-9">
			...
			<div id="tape-container" style="margin-top:0px; margin-left:-15px;" class="isotope">
				...
				<?php if ($my_query->have_posts()) : while ($my_query->have_posts()) : $my_query->the_post(); ?>
						...
						<?php $children = get_children(array('post_parent' => $post->ID, 'post_type' => 'sgw-poetry-readings'));
						...
						<?php if ($num_children == 0) : ?>
							...
							if ($post->post_parent > 0) {
								...
								if ($post->post_parent != $post_parent_prev) $tape_num++;
									...
									$tape_num = 1;
									$post_parent_prev = $post->post_parent;
								}
								...
								if ($date_cal == "") {
									...
									$date_cal = preg_match('/\d{4}/', $date_reading[0], $matches);
									$date_cal = $matches[0];
								}

								$date_reading = get_post_meta($post->post_parent, 'date-of-reading');
								preg_match('/\d{4}/', $date_reading[0], $matches);
								$year = $matches[0];
							} else {
								$tape_num = 1;
								$print_tape_num = 0;
